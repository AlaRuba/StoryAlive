...
@interface AnimatedPage26Cricket ()
...
@implementation AnimatedPage26Cricket
...
- (CGPathRef)newRandomPathWithStartingPoint:(CGPoint)firstPoint
{
    ...
    for ( i = 1; i < count; i++)
    {
        ...
        if (i%2 == 1) {
            x+=15;//random() % ((NSInteger)(self.bounds.size.width) + 50);
        } else {
            x-=15;//random() % ((NSInteger)(self.bounds.size.width) + 50);
        }
        y=firstPoint.y;
...
- (void)generateGlowingSphereLayer
{
    ...
    NSImage* compositeImage = [NSImage imageNamed:@"page26_27_cricket.png"];
